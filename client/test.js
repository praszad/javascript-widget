                ...
                h.once('packet', function (t) {
                  if (!p)
                    if ('pong' === t.type && 'probe' === t.data) {
                      if (((u.upgrading = !0), u.emit('upgrading', h), !h))
                        return;
        ...
        (e.encodePacket = function (e, r, i, a) {
          ...
          (e.decodePacket = function (t, r, n) {
            ...
            var i = new Uint8Array(t),
              ...
              s = f(t, 1);
            return (
              w && 'blob' === r && (s = new w([s])), { type: b[o], data: s }
            );
          }),
          ...
          (e.decodePayloadAsBinary = function (t, r, n) {
            ...
            for (var o = t, i = []; o.byteLength > 0; ) {
              for (
                var s = new Uint8Array(o), a = 0 === s[0], c = '', h = 1;
                255 !== s[h];
                h++
              ) {
    ...
    function (t, e, r) {
      (function (e) {
        ...
        var o,
          ...
          h = r(29),
          p =
            (r(3)('engine.io-client:websocket'), e.WebSocket || e.MozWebSocket);
      ...
      s(n.prototype),
        ...
        (n.prototype.onopen = function () {
          if ('/' !== this.nsp)
            if (this.query) {
              var t =
                'object' === o(this.query) ? p.encode(this.query) : this.query;
...
var genericStrings = {
  ...
  DEFAULT_ERROR_MESSAGE:
    'Something went wrong. Please try again or get in touch with the administrator',
  YES: 'Yes',
  NO: 'No',
...
var utils = {
  ...
  addPickadayScripts: function () {
    ...
    pickadayLink.href =
      'https://cdnjs.cloudflare.com/ajax/libs/pikaday/1.6.1/css/pikaday.min.css';
  ...
  sendDate: function () {
    ...
    if (
      EngtChat.speech.allowSpeechRecognition ||
      EngtChat.speech.allowAudioConversation
    ) {
      utils.removeClass(
        document.getElementById('engt-mic-btn'),
        'engt-hide-element'
      );
      utils.addClass(
        document.getElementById('engt-send-btn'),
        'engt-hide-element'
      );
    }
  },
  sendMsg: function (msg, focusTextarea) {
    if (utils.autoPopupTimeoutMinimized) {
      ...
      if (
        EngtChat.config.web_menu &&
        EngtChat.config.web_menu.call_to_actions &&
        EngtChat.config.web_menu.call_to_actions.length
      ) {
        utils.removeClass(
          document.getElementById('engt-composer-menu'),
          'engt-hide-element'
        );
      }
    }
    ...
    if (EngtChat.visibleAutoFill) {
      document.getElementById('txMessage').value = document.querySelector(
    ...
    var autofillEventListener = EngtChat.autofillData.getValues()
      .eventListenerFunc;
    if (EngtChat.config.show_password_input) {
      var message =
        msg ||
        document.getElementById('txMessagePassword').value.toString().trim();
    } else {
      var message =
        msg || document.getElementById('txMessage').value.toString().trim();
    }
    if (autofillOptions && message) {
      ...
      if (!foundOption) {
        ...
        EngtChat.addMessage(
          'Invalid Input. Please select an option from the dropdown only.',
          new Date(),
          'engt',
          undefined
        );
        ...
        if (autofillEventListener) {
          document
            .getElementById('txMessage')
            .removeEventListener('input', autofillEventListener);
          document
            .getElementById('txMessage')
            .removeEventListener('click', autofillEventListener);
        }
    ...
    if (message != '') {
      if (gotUid == true) {
        ...
        document.getElementsByClassName(
          'engt-sheet-header-with-presence'
        )[0].style.padding = '4px';
...
window.EngtChat = {
  ...
  selectedDate: '',
  status: { current: undefined, last: undefined, sessionKey: undefined },
  inputMode: 'input',
  speech: {
  ...
  open: function () {
    ...
    utils.removeClass(
      document.getElementById('engt-launcher-button'),
      'engt-launcher-active'
    );
    utils.addClass(
      document.getElementById('engt-launcher-button'),
      'engt-launcher-inactive'
    );
    if (this.config.chat_close_button_position === 'BOTTOM') {
      utils.addClass(
        document.getElementById('engt-close-button'),
        'engt-launcher-active'
      );
      utils.removeClass(
        document.getElementById('engt-close-button'),
    ...
    if (document.getElementById('engt-conversation') != undefined) {
      utils.removeClass(
        document.getElementById('engt-conversation'),
        'engt-inactive'
      );
      utils.addClass(
        document.getElementById('engt-conversation'),
    ...
    if (EngtChat.config.header_description) {
      document.getElementById('engt-sheet-header').style.height =
        70 +
        document.getElementById('engt-sheet-header-description').clientHeight +
        'px';
      document.getElementsByClassName('engt-sheet-content')[0].style.top =
        58 +
        document.getElementById('engt-sheet-header-description').clientHeight +
        'px';
      document.getElementsByClassName(
        'engt-sheet-content-auth-node'
      )[0].style.paddingTop =
        20 +
        document.getElementById('engt-sheet-header-description').clientHeight +
        'px';
    ...
    if (
      EngtChat.config.auto_pop_up_web_chat_type === 'MINIMIZED' &&
      EngtChat.config.setType === 1
    ) {
      EngtChat.config.setType += 1;
      var engtContainer = document.getElementById('engt-conversation');
      var containerHeight =
        62 + document.getElementById('engt-sheet-header').clientHeight;
  ...
  close: function () {
    ...
    if (
      utils.hasClass(
        document.getElementById('engt-launcher'),
        'engt-hide-element'
      ) &&
      EngtChat.config.auto_pop_up_web_chat_type === 'MINIMIZED'
    ) {
  ...
  addMessage: function (message, timestamp, user, type) {
    ...
    message =
      '<p style="white-space: pre-line;" dir=' +
      utils.direction +
      '>' +
      message +
      '</p>';
    ...
    var msgHtml =
      '<div class="engt-comment-body-container ' +
      carouselclass +
      '"><div class="engt-comment-body ' +
      msgClass +
      '">';
    ...
    if (this.config.bot_avatar_image_url) {
      ...
      if (EngtChat.config.carouselLayout === 'continuous_scroll')
        engt_comment_width_class = 'engt-carousel-comment-width';
        ...
        style.type = 'text/css';
        style.innerHTML =
          '#engt-container .engt-carousel-single-card-width-' +
          EngtChat.config.carouselWidth +
          '{max-width: ' +
          cardWidth +
          'px;}';
        document.getElementsByTagName('head')[0].appendChild(style);
        engt_comment_width_class =
          'engt-carousel-single-card-width-' + EngtChat.config.carouselWidth;
      }
    }
    if (this.config.bot_avatar_image_url && type === 'carousel') {
      if (EngtChat.config.carouselLayout === 'continuous_scroll')
        engt_comment_width_class = 'engt-avatar-carousel-width';
      else if (EngtChat.config.carouselLayout === 'show_one') {
        if (EngtChat.config.cardType === 'list') {
          var cardWidth = EngtChat.config.carouselWidth;
        } else {
        ...
        style.type = 'text/css';
        style.innerHTML =
          '#engt-container .engt-avatar-carousel-single-card-width-' +
          EngtChat.config.carouselWidth +
          '{max-width: ' +
          cardWidth +
          'px;}';
        document.getElementsByTagName('head')[0].appendChild(style);
        engt_comment_width_class =
          'engt-avatar-carousel-single-card-width-' +
          EngtChat.config.carouselWidth;
      }
    }
    if (
    ...
    this.metadata.innerHTML =
      '<div class="engt-comment-metadata">' +
    ...
    if (EngtChat.config.edit_user_input && user == this.config.user) {
      EngtChat.removeAllElem(
        document.getElementsByClassName('engt-conversation-part__edit-btn')
      );
    ...
    div_message.insertBefore(
      document.getElementById('loader_message'),
      div_message.lastChild
    );
    ...
    if (type === 'carousel' && EngtChat.config.cardType !== 'list') {
      ...
      new Glider(document.querySelector(gliderClass), {
        ...
        exactWidth: true,
        arrows: { prev: prevClass, next: nextClass },
      });
    }
  },
  ...
  editResponse: function (e) {
    ...
    var defaultMsg =
      EngtChat.config.default_edit_response || 'I want to change the answer';
  ...
  addCarousel: function (templates, timestamp, user, type) {
    ...
    if (cardType === 'list') {
      ...
      if (headerTextColor) {
        ...
        document.querySelector('style').innerHTML +=
          '#engt-container .engt-list-carousel-header-button' +
          EngtChat.carouselTimeStamp +
          ':hover{color: white !important; background-color: ' +
          headerTextColor +
          '!important;}';
      }
      if (headerImage) {
        htmlText +=
          '<div class="engt-header-section" style="background-image: url(' +
          headerImage +
        ...
        if (subHeader) {
          htmlText +=
            '<p class="engt-list-carousel-text engt-header-text-subheading engt-text-white" ' +
            headerColor +
            '>' +
            subHeader +
            '</p>';
        ...
        if (headerButtons) {
          ...
          for (var j = 0; j < headerButtons.length; j++) {
            ...
            var value =
              headerButtons[j]['payload'] +
              ')(' +
              headerButtons[j]['title'] +
              ')(' +
              href;
            htmlText +=
              '<button ' +
              ...
              j +
              convTimestamp +
              '" class="engt-list-header-button engt-list-carousel-header-button' +
              EngtChat.carouselTimeStamp +
              ' engt-btncarousel' +
              convTimestamp +
              '" value="' +
        ...
        htmlText +=
          ...
          headerColor +
          '><b class="engt-break-word">' +
          header +
          '</b></p>';
        if (subHeader) {
          htmlText +=
            '<p class="engt-list-carousel-text engt-header-text-subheading" ' +
            headerColor +
            '>' +
            subHeader +
            '</p>';
      ...
      for (var i = 0; i < carouselCards.length; i++) {
        ...
        htmlText +=
          '<div class="' +
          borderClass +
          '"><div class="engt-list-inner-div">   <div class="' +
          listContentClass +
          '"><p class="engt-list-carousel-text"><b class="engt-break-word">' +
          carouselCards[i]['title'] +
          '</b></p>';
        if (carouselCards[i]['subtitle']) {
          htmlText +=
            '<p class="engt-list-carousel-text engt-list-carousel-subtitle">' +
            carouselCards[i]['subtitle'] +
            '</p>';
        }
        htmlText += '</div>';
        if (carouselCards[i]['image_url']) {
          htmlText +=
            ' <div class="engt-list-image"><img class="engt-carousel-list-image" src="' +
            carouselCards[i]['image_url'] +
            '"></div>';
        }
        htmlText += '</div>';
        ...
        if (buttons) {
          ...
          for (var j = 0; j < buttons.length; j++) {
            ...
            var value =
              buttons[j]['payload'] + ')(' + buttons[j]['title'] + ')(' + href;
            ...
            htmlText +=
              ...
              j +
              convTimestamp +
              '" class="engt-list-carousel-buttons engt-btncarousel' +
              convTimestamp +
              '" value="' +
              value +
              '">' +
      ...
      if (footerButtons) {
        for (var j = 0; j < footerButtons.length; j++) {
          ...
          var value =
            footerButtons[j]['payload'] +
            ')(' +
            footerButtons[j]['title'] +
            ')(' +
            href;
          ...
          htmlText +=
            ...
            '" class="' +
            btnClass +
            'engt-list-footer-button engt-buttoncarousel engt-btncarousel' +
            convTimestamp +
            '" value="' +
            value +
            '">' +
      ...
      htmlText +=
        '<div class="glider-contain"><div class="glider' +
        EngtChat.carouselTimeStamp +
        '">';
      for (var i = 0; i < carouselCards.length; i++) {
        var gradient = '';
        if (
          carouselCards[i].gradient_top_color !== '#FFFFFF' &&
          carouselCards[i].gradient_bottom_color !== '#FFFFFF' &&
          cardType === 'seamless'
        ) {
          backgroundColor = 'style="background-color: transparent!important"';
          gradient =
            'style="background-image:linear-gradient(' +
            carouselCards[i].gradient_top_color +
            ', ' +
            carouselCards[i].gradient_bottom_color +
            ')!important"';
        }
        var btnClass = '';
        ...
        if (carouselCards[i]['default_action']) {
          htmlText +=
            ...
            carouselCards[i]['default_action'] +
            '" + target="_blank">';
        }
        if (carouselCards[i]['image_url']) {
          htmlText +=
            ...
            backgroundColor +
            ' class="engt-img-' +
            carouselCards[i]['image_aspect_ratio'] +
            '"><img class="' +
            imageClass +
            '" src="' +
            carouselCards[i]['image_url'] +
          ...
          htmlText +=
            '<div ' +
            backgroundColor +
            ' class="engt-card-content engt-image-card-content-padding ' +
            cardContentClass +
            ' ' +
            noBtnCardContent +
            '">';
          htmlText +=
            '<p style="margin:0!important;white-space: pre-line;text-align:' +
            cardTextAlignment +
            '" dir=' +
            utils.direction +
            '><b class="engt-break-word">' +
          ...
          htmlText +=
            '<div ' +
            backgroundColor +
            ' class="engt-card-content engt-card-content-padding ' +
            cardContentClass +
      ...
      htmlText +=
        '</div><button role="button" aria-label="Previous"  id="glider-prev' +
        EngtChat.carouselTimeStamp +
      ...
      htmlText +=
        '<button role="button" aria-label="Next" id="glider-next' +
        EngtChat.carouselTimeStamp +
        '" class="glider-next ' +
        nextButtonClass +
        '" ' +
    ...
    var elem = document.getElementsByClassName(
      'engt-btncarousel' + convTimestamp
    );
  ...
  addLocation: function (templates, timestamp, user, type) {
    ...
    if (templates['image_url']) {
      htmlText +=
        'Your Location:<img src=' + templates['image_url'] + ' width="250"/>';
  ...
  addVideo: function (video_url, timestamp, user, type) {
    ...
    if (video_url) {
      htmlText +=
        '<video class="engt-width-100" controls><source src="' +
        video_url +
  ...
  addAudio: function (audio_url, timestamp, user, type) {
    ...
    if (audio_url) {
      htmlText +=
        '<audio controls class="engt-max-width-240"><source src="' +
        audio_url +
  ...
  addImage: function (image_url, timestamp, user, type) {
    ...
    if (image_url) {
      htmlText +=
        "<img class='engt-width-100' src='" + image_url + "' alt='image'>";
  ...
  addButtonEventListener: function (elem, timeStamp) {
    for (var index = 0; index < elem.length; index++) {
      (function () {
        ...
        var href = data[2];
        var text = data[1];
        var message = { postback: postback, type: 'postback', text: text };
        if (href === '' || href === undefined) {
          elem[index].addEventListener(
            'click',
            function () {
  ...
  addMediaNode: function (mediaData, timeStamp, user, type) {
    ...
    if (mediaData.media_type === 'image' && mediaData.media_url) {
      htmlText +=
        "<img class='engt-media-node-image' src='" +
        mediaData.media_url +
        "' alt='image'>";
    } else if (mediaData.media_type === 'video' && mediaData.media_url) {
      htmlText +=
        '<video class="engt-media-node-image" controls><source src="' +
        mediaData.media_url +
        '" type="video/mp4"></video>';
    ...
    if (mediaData.buttons) {
      ...
      for (var j = 0; j < buttons.length; j++) {
        ...
        htmlText +=
          ...
          j +
          convTimestamp +
          '" class="engt-mediabtn-listener' +
          convTimestamp +
          EngtChat.mediaIndex +
          ' engt-buttoncarousel engt-buttoncarousel-last engt-media-button ' +
          videoButtonClass +
          '" value="' +
          value +
          '">' +
    ...
    var elem = document.getElementsByClassName(
      'engt-mediabtn-listener' + convTimestamp + EngtChat.mediaIndex
    );
    EngtChat.addButtonEventListener(elem, timeStamp);
    EngtChat.mediaIndex++;
  },
  skipLogin: function () {
    var message = { type: 'identity', action: { skip_login: true } };
    EngtChat.sendMessage(message);
    utils.removeClass(
      document.getElementById('engt-conversation-window'),
      'engt-hide-element'
    );
    utils.addClass(
      document.getElementById('engt-auth-node-window'),
      'engt-hide-element'
    );
  },
  showHideMenu: function () {
    var element = document.getElementById('engt-menu-dropdown-container');
    var hamburgerMenuSection = document.getElementById(
      'engt-hamburger-menu-section'
    );
  ...
  getDropdownMenuHtml: function () {
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i].type === 'postback') {
        html +=
          '<li><a onclick=\'EngtChat.triggerMenuOption("' +
          data[i].title +
          '","' +
          data[i].type +
          '","' +
  ...
  triggerMenuOption: function (title, type, payload) {
    ...
    EngtChat.resetEverything();
    utils.addClass(
      document.getElementById('engt-menu-dropdown-container'),
      'engt-hide-element'
    );
    utils.removeClass(
      document.getElementById('engt-hamburger-menu-section'),
      'is-active'
    );
    EngtChat.changeInputMode('input');
    if (type === 'postback' && payload) {
      var message = { postback: payload, type: type, text: title };
  ...
  authLogin: function () {
    ...
    var loginInputFields = document.querySelectorAll(
      '.engt-text-sign-in-fields input'
    );
    var loginInputData = {};
    var empty_field_array = [];
    for (var field = 0; field < loginInputFields.length; field++) {
      if (
        loginInputFields[field].value == '' ||
        loginInputFields[field].value == null
      ) {
        utils.removeClass(
          document.getElementById('authErrorMessage'),
          'engt-hide-element'
      ...
      loginInputData[loginInputFields[field].name] =
        loginInputFields[field].value;
    }
    if (empty_params === true) {
      document.getElementById('authErrorMessage').innerHTML =
        'Input mandatory for ' + empty_field_array.join(', ') + '.';
      return;
    } else {
      document.getElementById('engt-sign-in-btn').disabled = true;
      if (Object.keys(loginInputData).length) {
        var message = { type: 'identity', params: loginInputData, action: {} };
        EngtChat.sendMessage(message);
      }
    }
  },
  authNodeValidation: function (auth_node_data) {
    if (
      ...
      !auth_node_data['login_success']
    ) {
      var authDiv = document.getElementById('engt-sign-in-content');
      var auth_btn_text = 'Sign In';
      auth_attr_map = {};
      var htmlText =
        '<div class="engt-auth-login-form"><div class="engt-text-heading-sign-in">' +
        auth_node_data['title'] +
        '</div><div class="engt-text-sub-heading-sign-in">' +
        auth_node_data['subtitle'] +
        '</div>';
      for (var attr in auth_node_data['attributes']) {
        var auth_type = '';
        auth_attr_map[auth_node_data['attributes'][attr]['attribute_id']] =
          auth_node_data['attributes'][attr]['display_name'];
        if (auth_node_data['attributes'][attr]['attribute_type'] == 'mobile') {
          auth_node_data['attributes'][attr]['attribute_type'] = 'number';
        }
        if (
          auth_node_data['attributes'][attr]['attribute_type'] === 'password'
        ) {
          auth_type = 'password';
        }
        htmlText +=
          '<div class="engt-text-sign-in-fields"><input required name="' +
          auth_node_data['attributes'][attr]['attribute_id'] +
          '" type="' +
          auth_type +
          '" value="" class="engt-login-input"><label class="engt-login-input-label">' +
          auth_node_data['attributes'][attr]['display_name'] +
          '*</label></div>';
      }
      ...
      htmlText +=
        ...
        EngtChat.config.themecolor +
        ' !important;border-color: ' +
        EngtChat.config.themecolor +
        ' !important;  color:' +
        this.config.themetextcolor +
        ' !important;" id="engt-sign-in-btn" onclick="EngtChat.authLogin();">' +
        auth_btn_text +
      ...
      for (var field = 0; field < loginInputFields.length; field++) {
        loginInputFields[field].addEventListener(
          'keypress',
          function (e) {
            if (e.keyCode === 13) {
              EngtChat.authLogin();
      ...
      if (auth_node_data['skip_login'] === true) {
        utils.removeClass(
          document.getElementsByClassName('engt-sign-in-skip-login')[0],
          'engt-hide-element'
        );
      }
    } else {
      if (auth_node_data['login_success'] === true) {
        if (EngtChat.currentUserId !== auth_node_data['user_id']) {
          ...
          if (localStorage[this.config.bot_key + 'newSessionKey']) {
            localStorage[this.config.bot_key + 'newSessionKey'] =
              auth_node_data['user_id'];
          }
        ...
        auth_node_data['login_success'] === false &&
        (auth_node_data['login_failed_msg'] ||
          auth_node_data['invalid_param_msg'])
      ) {
        document.getElementById('authErrorMessage').innerHTML =
          auth_node_data['invalid_param_msg'] ||
          auth_node_data['login_failed_msg'];
  ...
  addOptions: function (obj, timestamp, user, type) {
    ...
    var msgHtml = '';
    var message = '';
    var addPaddingClass = 'engt-options-only-date';
    ...
    if (obj.message) {
      ...
      message =
        ...
        '</p>';
      msgHtml =
        '<div class="engt-comment-body-container" style="width:max-content;"><div class="engt-comment-body ' +
        msgClass +
        '">';
      msgHtml = msgHtml + message + '</div></div>';
      addPaddingClass = '';
    ...
    msgContainer.innerHTML = msgHtml;
    msgHtml = this.addSenderIcon(msgContainer);
    var metadataContainer = document.querySelector(
      '.engt-comment-metadata-container'
    );
    if (metadataContainer) {
      if (this.status.last !== this.status.current) {
        ...
        utils.addClass(
          metadataContainer,
          'engt-comment-metadata-container-static'
        );
      } else {
    ...
    if (!obj.message) {
      optioncontainer.appendChild(this.metadata);
    }
    for (i = 0; i < options.length; i++) {
      ...
      if (options[i].image_url) {
        option_string +=
          '<img class="engt_opt-image" src="' + options[i]['image_url'] + '"/>';
      }
          ...
          if (type == 'location') {
            ...
            elem[i].addEventListener(
              ...
              function () {
                ...
                EngtChat.addMessage(text, timestamp, 'guest', 'undefined');
                var msgContainer = document.querySelector(
                  '.engt-sheet-content'
                );
                utils.scrollToEnd(msgContainer);
                EngtChat.sendMessage(message);
  ...
  addAutoFillPopupWithSearch: function (obj) {
    ...
    var sendSearchEvent = function () {
      ...
      lengthOfOriginalOptionsData = EngtChat.autofillOriginalData.getValues()
        .allOptions.length;
      if (
        lengthOfOriginalOptionsData < 250 &&
        lengthOfOriginalOptionsData !== 0
      ) {
        EngtChat.searchOptionsInUI(txMessage.value);
      } else {
        var textAlignClass =
          utils.direction === 'rtl' ? 'engt-text-right' : 'engt-text-left';
        EngtChat.autofillOriginalData.setOriginalSearchStr(
          txMessage.value.trim()
        );
        var data = { type: 'autocomplete', text: searchStr };
  ...
  renderAutoFillPopupSearchOptions: function (obj) {
    ...
    allOptions.forEach(function (opt) {
      if (i === 0) {
        popuphtml +=
          '<button tabindex="' +
          i +
          '" class="engt-autofill-btn engt-autofill-active ' +
          textAlignClass +
          '" dir=' +
          utils.direction +
          '>' +
    ...
    this.autofillData.update(
      allOptions,
      this.autofillData.getValues().eventListenerFunc
    );
  },
  addAutofillPopup: function (obj) {
    ...
    var filterOptions = function () {
      ...
      var onAutofillBtnClick = function () {
        ...
        document.getElementById(
          'txMessage'
        ).selectionStart = document.getElementById('txMessage').selectionEnd =
          text.length;
        self.toggleAutoFill(false);
        document.getElementById('txMessage').focus();
      };
    ...
    document
      .getElementById('txMessage')
      .addEventListener('click', filterOptions);
  },
  searchOptionsInUI: function (searchText) {
    var originalText = EngtChat.autofillOriginalData.getValues()
      .originalSearchStr;
    ...
    if (counter === originalText.length && searchText != '') {
      for (var i = 0; i < optionsData.length; i++) {
        ...
        for (var j = 0; j < searchText.length; j++) {
          if (
            optionsData[i].text
              .toLowerCase()
              .indexOf(searchText[j].toLowerCase()) !== -1
          ) {
            check++;
          }
        }
      ...
      var textAlignClass =
        utils.direction === 'rtl' ? 'engt-text-right' : 'engt-text-left';
      EngtChat.autofillOriginalData.setOriginalSearchStr(
        searchTextForStorage.trim()
      );
      var data = { type: 'autocomplete', text: searchText.toString().trim() };
      EngtChat.sendMessage(data);
    }
  },
  autofillData: {
  ...
  linkify: function (inputText) {
    ...
    replacedText = replacedText.replace(
      replacePattern2,
      '$2<a href="http://$3" target="_blank">$3</a>'
    );
  ...
  socketEventHandler: function (launch_flow) {
    ...
    document.getElementsByClassName(
      'engt-icon-name-container'
    )[0].style.padding = '20px 20px 1px 0px';
    ...
    socket.on('welcome_msg', function (data) {
      ...
      for (var ack in ackMessageMap) {
        ...
        var scheduleTimeout = setTimeout(function () {
          if (document.getElementById('inform_message')) {
            document.getElementById(
              'inform_message'
            ).style.display = botResponse ? 'none' : 'block';
          }
        }, 15e3);
      ...
      if (
        EngtChat.config.enableIpAddress &&
        EngtChat.config.ipAddress &&
        localStorage
      ) {
        var ipAddress = localStorage.getItem(
          EngtChat.config.bot_key + 'ipAddress'
        );
      ...
      if (EngtChat.config.repeatFlowOnClick !== true) {
    ...
    socket.on('bot_response', function (data) {
      ...
      document.getElementById('loader_message').style.display = 'none';
      if (document.getElementById('inform_message')) {
        document.getElementById('inform_message').style.display = 'none';
      }
      data = JSON.parse(data);
      var mssgId = data['message_id'];
      if (mssgId === lastMssgId) {
      ...
      if (data['campaign_key']) {
        ...
        if (EngtChat.config.is_debug) {
          console.log(
            'status packet sent for campaign -> ',
            data['campaign_key']
          );
        }
      }
      ...
      if (
        ...
        msg_type !== 'audioUpload' &&
        msg_type !== 'processedAudio'
      ) {
        var prevNodeKey = EngtChat.prevNodeData.node_key;
        if (
          prevNodeKey.indexOf('_') === -1 ||
          prevNodeKey.split('_')[0] !== data['node_key']
        ) {
          EngtChat.prevNodeData = '';
        } else {
      ...
      if (msg_type != 'typing_indicator') {
        ...
        if (EngtChat.config.disableInputOnOptions) {
          document.getElementById('txMessage').disabled = false;
          utils.addClass(
            document.getElementsByClassName('textarea-overlay ')[0],
            'engt-hide-element'
          );
        }
      }
      if (message.voice_url && EngtChat.speech.allowVoiceOutput) {
        ...
        if (
          EngtChat.inputMode === 'waiting_for_response' ||
          EngtChat.inputMode === 'speaking'
        ) {
      ...
      if (
        !message.voice_url &&
        EngtChat.speech.allowVoiceOutput &&
        EngtChat.inputMode === 'waiting_for_response'
      ) {
        clearTimeout(EngtChat.speech.nonVoiceTimeout);
      ...
      if (msg_type == 'typing_indicator') {
        ...
        if (message.enable_autofill && message.enable_search) {
          ...
          message.enable_autofill &&
          (!oldIOS || options_data.options.length > 10)
        ) {
          EngtChat.speech.pauseAfterCurrentAudio = true;
          EngtChat.enableAutoFill();
        ...
        msg_type === 'autocomplete' &&
        EngtChat.autofillData.eventListenerFunc
      ) {
        var options_data = message['data'];
        ...
        var htmlLinkedText =
          '<a href="' + url + '" target="_blank">' + urlPrefixMessage + '</a>';
  ...
  initEventHandler: function () {
    document.getElementById('engt-launcher-button').onclick = function (e) {
      if (EngtChat.config.initFunction) {
        var getUserAttributes = new Function(EngtChat.config.initFunction);
        var attributes = getUserAttributes();
        utils.mergeConfig(EngtChat.config.user_attributes, attributes);
      }
      EngtChat.open();
      EngtChat.socketEventHandler();
    ...
    document.getElementById('engt-auto-popup-container').onclick = function (
      e
    ) {
      if (e.target.className.indexOf('pop-up-close') != -1) {
        utils.addClass(
          document.getElementById('engt-auto-popup-container'),
          'engt-hide-element'
        );
      } else {
        EngtChat.open();
      ...
      if (keyCode == 13 && !e.shiftKey) {
        ...
        if (EngtChat.visibleAutoFill) {
          ...
          if (activeBtn.tabIndex === inputs.length - 1) {
            ...
            document.getElementById('engt-autofill-popup').scrollTop =
              nextInput.offsetTop - 5;
          }
          if (nextInput) {
    ...
    if (document.getElementById('txMessage') != undefined) {
      ...
      document.getElementById(
        'txMessagePassword'
      ).onkeydown = fncTxMessageKeydown;
      document
        .getElementById('engt-send-btn')
        .addEventListener('click', utils.sendMsg.bind(null, null, true));
    }
    document.getElementById('engt-composer-refresh').addEventListener(
      'click',
      function () {
        var msgDivs = document.querySelectorAll(
          '.engt-conversation-part:not(#loader_message)'
        );
  ...
  resetEverything: function (params) {
    ...
    utils.addClass(
      document.getElementById('engt-upload-loader'),
      'engt-hide-element'
    );
    EngtChat.toggleDatePicker(false);
  ...
  sendMessage: function (msg, type) {
    ...
    var autofillEventListener = EngtChat.autofillData.getValues()
      .eventListenerFunc;
    if (
      msg.type !== 'autocomplete' &&
      (autofillOptions || autofillEventListener)
    ) {
  ...
  getFontFamily: function (fontFamily, fontWeight) {
    ...
    if (
      ...
      fontFamilyCls =
        '#engt-container .engt-chatbox, .engt-opt-buttons, .engt-popup-text{font-family: ' +
        themeFontOptions[fontFamily] +
  ...
  init: function (config) {
      ...
      xhttp.onreadystatechange = function () {
        if (this.readyState == 4) {
          if (this.status == 200) {
            ...
            EngtChat.showConsoleLog(
              'Error in getting Bot Configs. Response: ' +
                JSON.stringify(xhttp.responseText)
            );
          }
        }
      };
    ...
    var params = [
      'SETTINGS.SELECT_AN_OPTION',
      'SETTINGS.BROWSE_FILE_TO_UPLOAD',
      'SETTINGS.SUBMIT',
      'SETTINGS.CONTINUE',
      'SETTINGS.INVALID_INPUT',
      'SETTINGS.PICK_A_DATE',
      'SETTINGS.START_TYPING_TO_FILTER',
      'SETTINGS.WRITE_A_REPLY',
      'SETTINGS.SEND_A_MESSAGE',
      'SETTINGS.OK',
      'SETTINGS.CANCEL',
      'SETTINGS.DID_THIS_ANSWER_HELP',
      'SETTINGS.DEFAULT_ERROR_MESSAGE',
      'SETTINGS.YES',
      'SETTINGS.NO',
    ];
    ...
    xhttp2.open(
      'POST',
      this.config.server + '/web/generic-strings/' + this.config.bot_key + '/',
      false
    );
    ...
    if (genericStringResponse && genericStringResponse.status) {
      ...
      genericStrings.SELECT_AN_OPTION =
        genericStringMap['SETTINGS.SELECT_AN_OPTION'];
      genericStrings.BROWSE_FILE_TO_UPLOAD =
        genericStringMap['SETTINGS.BROWSE_FILE_TO_UPLOAD'];
      genericStrings.CANCEL = genericStringMap['SETTINGS.CANCEL'];
      genericStrings.PLACE_HOLDER_PICK_A_DATE =
        genericStringMap['SETTINGS.PICK_A_DATE'];
      genericStrings.PLACE_HOLDER_START_TYPING_TO_FILTER =
        genericStringMap['SETTINGS.START_TYPING_TO_FILTER'];
      genericStrings.PLACE_HOLDER_WRITE_A_REPLY =
        genericStringMap['SETTINGS.WRITE_A_REPLY'];
      genericStrings.SEND_A_MESSAGE =
        genericStringMap['SETTINGS.SEND_A_MESSAGE'];
    ...
    this.successCBBotConfig = function (resp) {
      ...
      this.config.website_url = resp.website_url;
      if (resp.theme_color) {
      ...
      this.closeButtonClass =
        resp.chat_close_button_position === 'BOTTOM' ? '' : 'engt-visible';
      this.config.chat_bubble_color =
        resp.chat_bubble_color || resp.theme_color;
      this.config.button_selector_color =
        resp.button_selector_color || '#1AABED';
      this.config.chat_bubble_colortextcolor =
        resp.chat_bubble_text_color ||
        utils.getSaturation(this.config.chat_bubble_color);
      this.config.button_selector_colortextcolor = utils.getSaturation(
        this.config.button_selector_color
      );
      ...
      if (
        this.config.web_menu &&
        this.config.web_menu.call_to_actions &&
        this.config.web_menu.call_to_actions.length
      ) {
      ...
      this.config.disableInputOnOptions = resp.block_send_message_with_options
        ? true
        : false;
      var EngtSpeechRecognition =
        window.SpeechRecognition || window.webkitSpeechRecognition || null;
      var getUserMediaSupported =
      ...
      EngtChat.speech.allowSpeechRecognition =
        EngtSpeechRecognition &&
        (resp.voice_input_enabled || resp.audio_conversation_enabled);
      EngtChat.speech.allowAudioConversation =
        !EngtChat.speech.allowSpeechRecognition &&
        resp.audio_conversation_enabled &&
        getUserMediaSupported;
      EngtChat.speech.allowContinuousMode =
        (EngtChat.speech.allowSpeechRecognition ||
          EngtChat.speech.allowAudioConversation) &&
        resp.enable_continuous_voice;
      EngtChat.speech.allowVoiceOutput =
        resp.audio_conversation_enabled && !!Audio;
      ...
      if (EngtChat.speech.allowVoiceOutput) {
        ...
        EngtChat.speech.audioObj.addEventListener('ended', function () {
          ...
          if (EngtChat.speech.voiceQueue.length) {
            ...
            EngtChat.speech.allowContinuousMode &&
            EngtChat.inputMode !== 'input' &&
            !EngtChat.speech.pauseAfterCurrentAudio
          ) {
      ...
      if (this.config.remove_branding == true) {
        ...
        var engt_powered =
          '<a href="http://www.engati.com/#bot" target="_blank"><span class="engt-text-red">Powered by Engati - engati.com</span></a>';
      } else {
        var engt_powered =
          '<a href="' +
      ...
      if (auto_pop_up_type === 'closed_callout') {
        ...
        if (
          (resp.auto_enable_pop_up_delay === 0 ||
            resp.auto_enable_pop_up_delay > 0) &&
          resp.pop_up_message
        ) {
      ...
      if (resp.header_text_color) {
        ...
        this.config.themetextcolor = utils.getSaturation(
          this.config.themecolor
        );
      }
      if (resp.theme_font && resp.theme_font !== 'helvetica') {
        var fontParams = EngtChat.getFontFamily(
          resp.theme_font,
          resp.font_weight
        );
      }
      ...
      style.type = 'text/css';
      style.innerHTML =
        '#engt-container .engt-comment-by-user .engt-comment-body-container{background:' +
        this.config.chat_bubble_color +
        '!important;color:' +
        this.config.chat_bubble_colortextcolor +
        '!important;}#engt-container .engt-auto-popup-container:before{border-color: transparent ' +
        this.config.themecolor +
        '!important;}#engt-container .engt-popup-caret:after {border-top-color:' +
        this.config.themecolor +
        '!important;}.engt-popup-text{border: solid 2px ' +
        this.config.themecolor +
        '!important;} .pop-up-close{color: ' +
        this.config.themecolor +
        '!important;} #engt-container .engt-comment-by-user .engt-comment-caret:after{border-left-color:' +
        this.config.chat_bubble_color +
        ...
        this.config.button_selector_color +
        ' !important; color: ' +
        this.config.button_selector_colortextcolor +
        ' !important;} #engt-container .engt-seamless-card-button:hover{color:' +
        this.config.button_selector_color +
        '}.engt-separate-card-button, .engt-media-button, .engt-list-footer-button, .engt-list-carousel-buttons{color: ' +
        this.config.button_selector_color +
        ' !important;} .engt-list-carousel-buttons{border-color: ' +
        this.config.button_selector_color +
      ...
      var img_icon_close = '';
      if (
        this.config.use_icon_dimensions &&
        this.config.chat_widget_position == 'bottomleft'
      ) {
        img_icon_open =
          '><img src="' +
          this.config.icon_url +
          '" style="position:fixed; bottom:0"></div>';
        img_icon_close = 'style="position:fixed; bottom:0"';
      } else if (
        this.config.use_icon_dimensions &&
        this.config.chat_widget_position == 'bottomright'
      ) {
        img_icon_open =
          '><img src="' +
          this.config.icon_url +
        ...
        img_icon_open =
          'style="background-image:url(' + this.config.icon_url + ')"></div>';
        img_icon_close = '';
      }
      div_root.innerHTML =
        '<div id="engt-container" class="engt-container engt-reset engt-acquire"><div id="engt-launcher" class="engt-launcher ' +
        enable_launcher +
        ' "><div id="engt-auto-popup-container" class="engt-auto-popup-container ' +
        engt_auto_container_position +
        '" style="display:none;"><div class="engt-popup-text ' +
        engt_popup_text_position +
        '"><span class="pop-up-close">&times;</span><span class="pointer-cursor">' +
        pop_up_message +
        ...
        this.config.themetextcolor +
        '!important;font-size:18px;" dir=' +
        utils.direction +
        '></b></div><b class="engati-intialization" id="engati-intialization" style="color:' +
        this.config.themetextcolor +
        '!important">Initializing..</b></div><i class="engt-sheet-header-close ' +
        this.closeButtonClass +
        ...
        this.config.text_area_color +
        ';"><div class="engati-composer-content ' +
        engt_composer_padding +
        '"><div id="engt-autofill-popup" class="engt-hide-element" style="left:' +
        autofillLeft +
        ' !important;"></div><button id="datepicker" class="hide-datepicker engt-datepicker ' +
        engt_refresh_date_picker +
        ...
        dropDownMenuHtml +
        '</div><div id="engt-composer-menu" class="' +
        engt_menu_display +
        ...
        '" dir=' +
        utils.direction +
        ' style=" top: 5px;"></textarea>' +
        engt_chat_btns +
        '<div id="engt-upload-loader" class="engt-upload-loader engt-hide-element" style="background: ' +
        this.config.text_area_color +
        ';border-top-color: ' +
        EngtChat.config.themecolor +
      ...
      if (this.config.e === 'd') {
        ...
        gliderCompat.src =
          'https://dev.engati.com/static/js/glider-compat.min.js';
        ...
        gliderCompat.src =
          'https://app.engati.com/static/js/glider-compat.min.js';
        ...
        gliderCompat.src =
          'http://localhost:5010/static/js/glider-compat.min.js';
        ...
        gliderCompat.src =
          'https://qa.engati.com/static/js/glider-compat.min.js';
        ...
        gliderLink.href =
          'https://staging.engati.com/static/css/glider.min.css';
        gliderScript.src = 'https://staging.engati.com/static/js/glider.js';
        gliderCompat.src =
          'https://staging.engati.com/static/js/glider-compat.min.js';
        hamburgerLink.href =
          'https://staging.engati.com/static/css/hamburger.css';
      } else if (this.config.e === 'op') {
        ...
        gliderCompat.src =
          this.config.server + '/static/js/glider-compat.min.js';
      ...
      utils.isSafari = /^((?!chrome|android).)*safari/i.test(
        navigator.userAgent
      );
      if (utils.isSafari) {
      ...
      if (resp.header_alignment === 'LEFT') {
        var headerImageNameContainer = document.getElementsByClassName(
          'engt-icon-name-container'
        )[0];
        var headerDescription = document.getElementById(
      ...
      if (this.config.use_icon_dimensions && resp.icon_url) {
        document.getElementById('engt-launcher-button').style.background =
          'none';
      }
      if (this.config.chat_widget_position == 'bottomleft') {
        document.getElementById('engt-launcher').style.left = '40px';
        document
          .getElementById('engt-conversation')
          .setAttribute(
            'style',
      ...
      if (resp.auto_pop_up_web_chat_type === 'CLOSED') {
        utils.autoPopupTimeout = setTimeout(function () {
          document.getElementById('engt-auto-popup-container').style.display =
            'block';
      ...
      ).innerHTML = this.config.bot_name;
      if (resp.header_description) {
        this.config.header_description = resp.header_description;
        document.querySelector('.engt-sheet-header-description').innerHTML =
          resp.header_description;
      }
      if (
        EngtChat.speech.allowSpeechRecognition ||
        EngtChat.speech.allowAudioConversation
      ...
      if (localStorage && localStorage[this.config.bot_key + 'newSessionKey']) {
        ...
        if (localStorage) {
          localStorage[
            this.config.bot_key + 'newSessionKey'
          ] = this.status.sessionKey;
          this.firstTime = true;
        }
      }
  ...
  addSenderIcon: function (msgContainer) {
    ...
    if (url && this.status.current === 'engt') {
      var senderIcon =
        '<div class="engt-conversation-sender-icon"><img src="' +
        url +
        '"></div>';
      if (
        this.status.last !== 'engt' ||
        this.chat_refreshed ||
        (this.config.agentAvatarImageUrl &&
          this.config.livechatMessageCount == 1)
      ) {
        msgHtml += senderIcon;
  ...
  createFileTile: function (file) {
    ...
    div.id = 'file-tile';
    div.innerHTML =
      '<span onclick="window.EngtChat.clearFile(true)">' +
      file.name +
      '<i></i></span>';
    ...
    if (
      utils.hasClass(engtcomposermenu, 'engt-hide-element') &&
      utils.hasClass(engtComposerRefresh, 'engt-hide-element')
    ) {
  ...
  uploadFile: function () {
    ...
    xhttp.setRequestHeader(
      'customInfo',
      JSON.stringify({ user_id: EngtChat.status.sessionKey, bot_ref: bot_ref })
    );
  ...
  changeInputMode: function (mode) {
    ...
    EngtChat.showConsoleLog(
      'state changed from ' + this.inputMode + ' to ' + mode
    );
  ...
  startRecording: function () {
    ...
    function onMediaSuccess(stream) {
      ...
      EngtChat.speech.mediaRecorder.ondataavailable = function (file) {
        if (
          EngtChat.speech.transaction_id &&
          file.size <= EngtChat.speech.MIN_AUDIO_RECORDING_FILE_SIZE
        ) {
          var uploadAudioObj = {
            file: file,
            fileUploadBaseURL: EngtChat.fileUploadBaseURL,
...
function showError(error) {
  ...
  var data = {
    type: 'location',
    message: { status: status, location: { lat: '', long: '' } },
  };
  EngtChat.sendMessage(data);
}
...
window.onclick = function (event) {
  if (
    ...
    event.target.parentNode.matches &&
    !event.target.parentNode.matches('.engt-composer-textarea')
  ) {
  ...
  if (
    ...
    !event.target.closest('#engt-hamburger-menu-section')
  ) {
    var element = document.getElementById('engt-menu-dropdown-container');
    var hamburgerMenuSection = document.getElementById(
      'engt-hamburger-menu-section'
    ...
    if (
      ...
      !utils.hasClass(element, 'engt-hide-element')
    ) {
      utils.addClass(element, 'engt-hide-element');
      utils.removeClass(hamburgerMenuSection, 'is-active');
    }
